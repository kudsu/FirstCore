...
namespace FirstCore.Models
{
    public class JWTTokenOptions
    {
        ...
        /// 秘钥，可以从配置文件中获取
        /// </summary>
        private static string emma = "cb4feff2fbdd2f8442ebf38aa3a9032be7e5da64bbde0bd7f77efe5df0ecbd1e";
        private static string zyh = "14002291F70839EE";
        /// <summary>
        /// 创建jwttoken,源码自定义
        /// </summary>
        ...
        public static string CreateToken(Dictionary<string, object> payLoad, int expiresMinute, Dictionary<string, object> header = null)
        {
            ...
            var hs256 = new HMACSHA256(Encoding.ASCII.GetBytes(emma));
            var encodedSignature = Base64UrlEncoder.Encode(hs256.ComputeHash(Encoding.UTF8.GetBytes(string.Concat(encodedHeader + zyh, ".", encodedPayload + zyh))));
        ...
        public static string CreateTokenByHandler(Dictionary<string, object> payLoad, int expiresMinute)
        {
            ...
            var jwt = new JwtSecurityToken(
                ...
                signingCredentials: new SigningCredentials(new SymmetricSecurityKey(Encoding.ASCII.GetBytes(emma)), SecurityAlgorithms.HmacSha256));
        ...
        public static bool Validate(string encodeJwt, Func<Dictionary<string, object>, bool> validatePayLoad)
        {
            ...
            var hs256 = new HMACSHA256(Encoding.ASCII.GetBytes(emma));
            //首先验证签名是否正确（必须的）
            success = success && string.Equals(jwtArr[2], Base64UrlEncoder.Encode(hs256.ComputeHash(Encoding.UTF8.GetBytes(string.Concat(jwtArr[0]+zyh , ".", jwtArr[1]+ zyh)))));
            if (!success)
            {
